...
if(isset($_GET['email'])){
    ...
    ts_orders.order_id, 
    ts_orders.email, 
    ts_orders.product_id, 
    ts_orders.size, 
    ts_orders.variant, 
    ts_orders.quantity, 
    ts_products.name,
    ts_products.base_price, 
    ts_products.description, 
    ts_products.images, 
    ts_orders.total_additional, 
    ts_orders.payment_status, 
    ts_orders.order_status, 
    (SELECT ts_payments.balance
     FROM ts_payments 
     WHERE ts_orders.order_id = ts_payments.order_id 
     ORDER BY ts_payments.id DESC 
     LIMIT 1) AS balance,
     (SELECT ts_payments.transaction_id
      FROM ts_payments
      WHERE ts_orders.order_id = ts_payments.order_id
      ORDER BY ts_payments.id ASC
      LIMIT 1 OFFSET 0) AS first_transaction_id,
     (SELECT ts_payments.transaction_id
      FROM ts_payments
      WHERE ts_orders.order_id = ts_payments.order_id
      ORDER BY ts_payments.id DESC
      LIMIT 1 OFFSET 0) AS last_transaction_id,
     (SELECT ts_payments.amount
      FROM ts_payments
      WHERE ts_orders.order_id = ts_payments.order_id
      ORDER BY ts_payments.id ASC
      LIMIT 1 OFFSET 0) AS first_transaction_amount,
     (SELECT ts_payments.amount
      FROM ts_payments
      WHERE ts_orders.order_id = ts_payments.order_id
      ORDER BY ts_payments.id DESC
      LIMIT 1 OFFSET 0) AS last_transaction_amount,
...
GROUP BY ts_orders.order_id DESC";
